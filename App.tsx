...
import { BotIcon, InfoIcon } from './components/Icons';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const hasSeenIntro = localStorage.getItem('hasSeenWhyBotIntro');
    if (!hasSeenIntro) {
  ...
  const handleCloseIntroModal = () => {
      ...
      localStorage.setItem('hasSeenWhyBotIntro', 'true');
  };
  ...
  return (
    <>
      <IntroModal isOpen={isIntroModalOpen} onClose={handleCloseIntroModal} />
      <div className="bg-sky-100 min-h-screen flex flex-col font-sans">
        <header className="bg-white shadow-md w-full p-4 flex items-center justify-between sticky top-0 z-10">
          ...
          <button
            ...
            className="text-gray-500 hover:text-sky-500 p-2 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-sky-400"
          ...
          <div className="flex-grow w-full max-w-3xl mx-auto overflow-y-auto pr-2">
            {chatHistory.length === 0 && !isLoading && (
              <div className="text-center text-gray-500 mt-20">
                  <BotIcon className="h-24 w-24 mx-auto text-sky-300" />
                  <p className="mt-4 text-xl">Ask me a question to get started!</p>
              </div>
            )}
